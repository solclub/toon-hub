    ...
    <div>
      <div className="w-full text-2xl">
        <div className="flex w-full items-center justify-between flex-col lg:flex-row">
          <div className="lg:w-2/5 mb-4 lg:mb-0 text-center lg:text-left">
            <h2 >My Collection</h2>
            <p className="font-sans text-gray-400 text-base">The portal is open click on one of your characters to toonify them!</p>
          </div>
          ...
          <div className="relative" key={x.mint}>
            <div
              className={classnames(
                "relative h-full transition duration-200 ease-in-out hover:-translate-y-1 hover:scale-105 border-[1px] border-gray-900 rounded-3xl overflow-hidden"
              )}
            >
              <Link href={"/profile/" + x.mint}>
                <Image
                  ...
                  height={300}
                ></Image>
                <div className="to-transparent absolute bottom-0 h-3/4 w-full bg-gradient-to-t from-black"></div>
                <div className="absolute bottom-5 w-full h-full text-3xl top-0 left-0 p-4 flex flex-col justify-between">
                  <div className="p-3 flex items-center text-2xl justify-center rounded-sm bg-[#ffe75c] w-fit h-fit aspect-square text-black">{x.tier}</div>
                  <div className="w-full flex justify-evenly text-center text-[#ffe75c]">
                    <span>{x.name.split(" #")[0]}<br />{x.name.split(" #")[1]}</span>
                    <span>Rank <br />{x.rudeRank}</span>
                  </div>
                </div>
              </Link>
            </div>
            {featuredItems.data && featuredItems.data.featuredNFTs.find((f) => f.mint == x.mint) && (
              <div
                className="tooltip btn-sm btn-circle btn absolute top-4 right-8 border-[#ffe75c] p-1"
