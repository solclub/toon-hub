...
export const getCrayonsBalance = async (wallet: string): Promise<number> => {
  try {
    const walletPubKey = new Web3PublicKey(wallet);
    const crayonsMintKey = new Web3PublicKey(env.NEXT_PUBLIC_GIBLATOONS_CRAYONS_MINT_KEY);
    const crayonaAta = await getAssociatedTokenAddress(
      crayonsMintKey,
      walletPubKey,
      false,
      TOKEN_2022_PROGRAM_ID
    );
    const balance = await connection.getTokenAccountBalance(crayonaAta);
    const balanceValue = balance.value.uiAmount?.toFixed(2);
    logWithTimestamp("getAssociatedTokenAddress");
    logWithTimestamp("getTokenAccountBalance");
    return Number(balanceValue || 0);
